...
export default function Profile() {
  ...
  const handleUpdateAddress = async () => {
    try {
      ...
      await axios.put(
        `${apiUrl}/api/wholesaler/edit-wholesaler`,
        {
  ...
  return (
    <View
      className={`${isDark ? 'bg-gray-900' : 'bg-gray-100'} flex-1 px-6 py-8`}
    >
      {}
      ...
      {user ? (
        <View
          ...
          } rounded-lg p-6 mb-8 shadow`}
        >
          <View className="flex items-center justify-center">
            <Text
              className={`text-2xl  font-semibold mb-1 px-2 ${
      ...
      <Modal visible={modalVisible} transparent animationType="slide">
        <View className="flex-1 bg-black/30 justify-center items-center">
          <View
            className={`${
              isDark ? 'bg-gray-800' : 'bg-white'
            } w-11/12 p-6 rounded-xl shadow-lg`}
          >
            <Text
            ...
            <TextInput
              className={`w-full h-12 px-4 mb-3 rounded-lg ${
                isDark
